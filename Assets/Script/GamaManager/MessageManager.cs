...
public class MessageManager : MonoBehaviour
{
    ...
    public GameObject Window,Window_T;
   ...
   // public Button submitBtn;
    ...
    {
        TransformData transformData = Window.GetComponent<TransformData>().GetBody();

        transformData.GetObjectValue<Text>("contentTxt").text = GetText;
        transformData.GetObjectValue<Text>("TitleTxt").text = title;
        Button submitBtn = transformData.GetObjectValue<Button>("OkBtn");
        submitBtn.gameObject.SetActive(false);
        GameManager.GetGameManager.OpenWindow(Window.transform);
    }
    ...
    public void WindowShowMessage(string GetText, Action action, string buttonName,bool isClsoe)
    {
        TransformData transformData = Window_T.GetComponent<TransformData>().GetBody();
        transformData.GetObjectValue<Text>("contentTxt").text = GetText;
        Button submitBtn = transformData.GetObjectValue<Button>("OkBtn");
        submitBtn.gameObject.SetActive(true);
        submitBtn.GetComponentInChildren<Text>().text = buttonName;
        ...
        submitBtn.GetComponent<ButtonClickAction>().ActionEvent += action;
        if(isClsoe)
        submitBtn.onClick.AddListener(() => { GameManager.GetGameManager.CloseWindow(Window_T.transform); });
        GameManager.GetGameManager.OpenWindow(Window_T.transform);
