...
public class GameManager : MonoBehaviour {
    ...
    public EventPatcher<JsonData> indexData = new EventPatcher<JsonData>();
    private void Update()
    {
    ...
    private void Start()
    {
        ...
        {
            foreach (GameObject itemwin in child.WindowList)
            {
                WinGroup.Add(itemwin.name,itemwin);
            }
        }

        if(IdelWindown)
        OpenWindow(IdelWindown,true);
        ActionEvent.Invoke();

    }

    [SerializeField]
    HttpModel index_http;
    public void SendData()
    {
        if (index_http == null)
            return;
        index_http.HttpSuccessCallBack.Addlistener(delegate(ReturnHttpMessage msg) 
        {
            if (msg.Code == HttpCode.SUCCESS)
            {
                indexData.Send(msg.Data["data"]);
            }
        });
    }
    ...
    Dic<string, GameObject> WinGroup = new Dic<string, GameObject>();
    ...
    public GameObject OpenWindow(string key)
    {
        GameObject obj = WinGroup.GetTValue(key);
        DotweenSetting(obj, true);
        return obj;
    }
    public GameObject CloseWindow(string key)
    {
        GameObject obj = WinGroup.GetTValue(key);
        DotweenSetting(WinGroup.GetTValue(key), false);
        return obj;
    }


    public void OpenWindow(Transform key,bool IsNoMrak=false)
    {
        DotweenSetting(key.gameObject, true, IsNoMrak);
    }
    ...
    public void DotweenSetting(GameObject go, bool isShow,bool IsNoMrak=false)
    {
        foreach (WindowDotweeCtrl child in WindowType)
        {
            if (child.WindowList.Contains(go))
            {
                ...
                {
                    winNum++;
                    child.OpenByDotween(go,IsNoMrak);
                }
                else
                {
                    winNum--;
    ...
    public void ClearChild(Transform parent)
        ...
        {
            if(childin.gameObject.activeInHierarchy)
            GameObject.Destroy(childin.gameObject);
        }
	    }
    public void ClearChild(List<GameObject> getbody)
    {
        foreach (GameObject childin in getbody)
        {
            GameObject.Destroy(childin.gameObject);
        }
    }
    ...
    //网络模块储存
    public DicBase<HttpModel> http_body = new DicBase<HttpModel>();
    public Transform IdelWindown;

    public bool IsHot(string id)
    {
        return id== "1" ? true : false;
    }

    public bool IsLargeMoney(string id)
    {
        if (Static.Instance.AppID == 0)
