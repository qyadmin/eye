...
public class TestListInspector : Editor
{
    ...
    private void DrawNameSingleList(Rect rect, int index, bool selected, bool focused)
    {
        ...
        if (ShowType == GetBackType.ImageNum)
            EditorGUI.PropertyField(new Rect(rect.x + rect.width / 3.5f, rect.y, rect.width / 3.5f, EditorGUIUtility.singleLineHeight), element.FindPropertyRelative("ShowTexture"), GUIContent.none);
        if (ShowType == GetBackType.RawImageNum)
            EditorGUI.PropertyField(new Rect(rect.x + rect.width / 3.5f, rect.y, rect.width / 3.5f, EditorGUIUtility.singleLineHeight), element.FindPropertyRelative("rawImage"), GUIContent.none);
        if (ShowType == GetBackType.RawIamge)
            EditorGUI.PropertyField(new Rect(rect.x + rect.width / 3.5f, rect.y, rect.width / 3.5f, EditorGUIUtility.singleLineHeight), element.FindPropertyRelative("rawImage"), GUIContent.none);
    ...
    private void DrawNameElement(Rect rect, int index, bool selected, bool focused)
    {
        ...
        if (ShowType == GetTypeValue.GetFromDropDown)
        {
            m_NameList.elementHeight = 40;
            EditorGUI.HelpBox(new Rect(rect.x + 110, rect.y, rect.width / 4, EditorGUIUtility.singleLineHeight), "从储存列表中按照DropDown值获取", MessageType.None);
            EditorGUI.PropertyField(new Rect(rect.x, rect.y + 20, 100, EditorGUIUtility.singleLineHeight), element.FindPropertyRelative("Name"), GUIContent.none);
            EditorGUI.PropertyField(new Rect(rect.width - 100, rect.y + 20, 100, EditorGUIUtility.singleLineHeight), element.FindPropertyRelative("DropDown"), GUIContent.none);
        }

        if (ShowType == GetTypeValue.GetFromToggleGroup)
        {
            m_NameList.elementHeight = 40;
            EditorGUI.HelpBox(new Rect(rect.x + 110, rect.y, rect.width / 4, EditorGUIUtility.singleLineHeight), "从储存列表中按照Toggle值获取", MessageType.None);
            EditorGUI.PropertyField(new Rect(rect.x, rect.y + 20, 100, EditorGUIUtility.singleLineHeight), element.FindPropertyRelative("Name"), GUIContent.none);
            EditorGUI.PropertyField(new Rect(rect.width - 100, rect.y + 20, 100, EditorGUIUtility.singleLineHeight), element.FindPropertyRelative("toggleGroup"), GUIContent.none);
        }
        // EditorGUI.PropertyField(new Rect(0, rect.y + 20, 20, EditorGUIUtility.singleLineHeight), element.FindPropertyRelative("IsC"), GUIContent.none);
